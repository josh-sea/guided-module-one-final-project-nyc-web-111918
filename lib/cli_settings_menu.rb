...
def settings_case
  ...
  when "4" #new Work (a actormovie connection)
    ...
    if movie_input == "exit"
      ...
      if bool == "y" || bool == "yes"
        actorID = Actor.actor_id_selector(actor_input)
        movieID = Movie.movie_id_selector(movie_input)
        if Work.find(movieID).actor_id == actorID
          puts "This connection already exists, trust me I checked my database"
          sleep 0.5
          puts "In fact, here it is!"
          sleep 0.5
          puts Work.find_by(movie_id: movieID, actor_id: actorID)
          sleep 0.5
          ...
          Work.create(movie_id: movieID, actor_id: actorID)
          sleep 0.5
          puts "Congratulations you did it!"
          puts "The actors in #{Movie.find(movieID).title} are:\n
          #{Movie.find(movieID).actors.map{|actor| actor.name}.join(", ")}"
          follow_up
        end
  ...
  when "5" #delete movie or actor instance
    ...
    puts "If this is incorrect please type back or enter the the movie title or actor name you want to delete."
    delete_input = gets_user_input
    if delete_input == "back"
      settings_menu
    elsif movie_or_actor == "movie" || movie_or_actor == "actor"
      if movie_or_actor == "movie"
        Movie.destroy(Movie.movie_id_selector(delete_input))
      else
        Actor.destroy(Actor.actor_id_selector(delete_input))
      end
    else
